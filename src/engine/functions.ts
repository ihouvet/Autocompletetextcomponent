...
reg({
  ...
  evaluate: (text, count) => {
    const s = toStr(text);
    return s.slice(Math.max(0, s.length - toNum(count)));
  },
});
...
reg({
  ...
  evaluate: (text, start, length) => {
    const s = toStr(text);
    const st = toNum(start) - 1;
    return s.slice(st, st + toNum(length));
  },
});
...
reg({
  ...
  evaluate: (search, text) => {
    const idx = toStr(text).indexOf(toStr(search));
...
reg({
  ...
  evaluate: (date, amount, unit) => {
    ...
    if (u === 'days') result.setDate(result.getDate() + n);
    else if (u === 'months') result.setMonth(result.getMonth() + n);
    else if (u === 'years') result.setFullYear(result.getFullYear() + n);
    else throw new Error(`Unknown unit: "${u}". Use "days", "months", or "years"`);
...
reg({
  ...
  evaluate: (date1, date2, unit) => {
    ...
    if (u === 'days') return Math.floor(diffMs / (1000 * 60 * 60 * 24));
    if (u === 'months') {
      return (d2.getFullYear() - d1.getFullYear()) * 12 + (d2.getMonth() - d1.getMonth());
    }
    if (u === 'years') return d2.getFullYear() - d1.getFullYear();
    throw new Error(`Unknown unit: "${u}". Use "days", "months", or "years"`);
  },
});
...
reg({
  ...
  evaluate: (num, decimals) => {
    const d = decimals !== undefined ? toNum(decimals) : 0;
    const factor = Math.pow(10, d);
    return Math.round(toNum(num) * factor) / factor;
  },
});
...
reg({
  ...
  evaluate: (num) => Math.floor(toNum(num)),
});
...
reg({
  ...
  evaluate: (num) => Math.ceil(toNum(num)),
});
...
reg({
  ...
  evaluate: (num) => Math.abs(toNum(num)),
});
...
reg({
  ...
  evaluate: (...args) => args.reduce((sum: number, v) => sum + toNum(v), 0),
});
...
reg({
  ...
  evaluate: (...args) => {
    ...
    return args.reduce((sum: number, v) => sum + toNum(v), 0) / args.length;
  },
});
...
reg({
  ...
  evaluate: (condition, thenValue, elseValue) =>
    toBool(condition) ? thenValue : elseValue,
});
...
reg({
  ...
  evaluate: (...args) => args.every(toBool),
});
...
reg({
  ...
  evaluate: (...args) => args.some(toBool),
});
